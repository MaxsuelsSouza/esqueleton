...
- **Duas modalidades de plano (`Plan.salesModality`):** **ONLINE** (padrão) is the self-service flow above. **PRESENCIAL** is sold by a platform rep — `Plan.setupFeeInCents` defines a one-time setup fee (e.g. R$378) collected manually/outside the system (cash, PIX in person — never through MercadoPago). A PRESENCIAL subscription is born `PENDING_SETUP` (the store stays offline even during the 7-day trial — `isStoreAvailable` blocks the trial while a `PENDING_SETUP` subscription exists). `POST /api/super/stores/:id/confirm-setup-fee` (super-admin) confirms the fee was received: the subscription flips straight to `ACTIVE` (store goes live immediately) and, if the plan is paid, creates the MercadoPago recurring PreApproval with `auto_recurring.start_date` = confirmation + 30 days (e.g. the R$97/month charge only starts then), returning a `paymentLink` for the owner to register a card. Self-service `POST /api/billing/subscribe` and `POST /api/super/stores/:id/payment-link` both reject PRESENCIAL plans (400) — reachable only through the rep-assisted flow (`POST /api/super/stores` → `confirm-setup-fee`).
