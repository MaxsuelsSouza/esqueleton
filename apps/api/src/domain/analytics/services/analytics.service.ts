// Agregação de métricas do dashboard — processa eventos e pedidos da loja
...
export async function computeAnalyticsSummary(prisma: PrismaClient, storeId: string) {
  const [events, soldOrders, pendingOrders, allOrders] = await Promise.all([
    prisma.productEvent.findMany({ where: { storeId }, orderBy: { createdAt: 'asc' } }),
    prisma.order.findMany({ where: { storeId, status: 'SOLD' } }),
    prisma.order.findMany({ where: { storeId, status: 'PENDING' } }),
